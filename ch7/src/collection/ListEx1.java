 ...
 *    
 *    
 *    List 인터페이스 : 순서가 존재함, 데이터 중복을 허용함 
 *    	ㄴ 구현클래스 : ArrayList, LinkedList, stack, vector...
 *    
 *    Set 인터페이스 : 순서를 유지하지 않음, 테이터 중복 허용하지 않음
 *    	ㄴ 구현클래스 : HashSet, TreeSet...
 *    
 *    Map 인터페이스 : key, value 의 쌍으로 이루어진 데이터 집합
 *           		순서을 유지하지 않음. key는 중복 안되고 value는 중복 허용
 *      ㄴ 구현클래스 : HashMap, TreeMap, Hashtable, Properties...
 *      
 *      
...
//		list3.add("참외");  <==런타임 오류남
		...
		// add(int index, 요소) => index 자리에 요소 추가하기
		list.add(3,6);
		System.out.println("특정 위치에 추가 후 "+list);
		
		//set(int index, 요소) => index 자리에 값을 요소로 변경하기
		list.set(3, 7);
		...
		// get(int index)
		System.out.println("특정 요소 위치 가져오기 "+list2.get(0));
		
		// indexOf(Object o)
		System.out.println("특정 요소의 위치 가져오기 "+list2.indexOf("서블릿"));
		
		// Collections.sort() : 오름차순이 기본, 내림차순을 원할 때는 기준을 추가로 제공하면 됨
